...
class ScrollLayout(WindowLayout):
    ...
    def allocateSize(self, allocated_size, requested_size, cells):
        ...
        Like any Sizeable object, the ScrollLayout object does obey to allocateSize
        and sets its size to the allocated size.  However, in the specific case
        of a ScrollLayout object, the allocated size only affects what will be
        visible on the screen once the GUI is drawn.  The content of the ScrollLayout
        object may very well be wider or higher than that, and that is
        perfectly valid: that is the very purpose of the ScrollLayout object: instead
        ...
        cells[0].allocateSize(SizeAllocation((0, 0), max_size))
